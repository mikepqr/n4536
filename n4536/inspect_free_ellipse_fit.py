...
el_nicmos_free = ellipse.ReadEllipse(d + "el_free/el_n4536_nicmos_fixed0freeepa.ascii", pix=0.075, telPA=246.74)

p.figure(0)
p.clf()
(ax1, ax2) = ellipse.PlotEllPA(el_irac1_free, xlog=True)
(ax1, ax2) = ellipse.PlotEllPA(el_nicmos_free, xlog=True, noErase=True, plotColorList=['r'])
...
p.suptitle("Free e/PA ellipse fit to IRAC1 image (black), NICMOS (red)")
p.show()
